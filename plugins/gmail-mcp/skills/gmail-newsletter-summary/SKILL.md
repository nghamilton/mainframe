...
### Step 3 -- Extract points from each email
 
**3a -- Locate the richest content region first**
 
Many newsletters concentrate their news in a predictable structural section rather than
spreading it through the whole email. Recognise these patterns and prioritise them:
...
For unlisted newsletters: scan for a numbered or bulleted list of 3--8 items -- this is
almost always the curated news section. Use it as your primary extraction source.
 
**3b -- Enrich with web_fetch where content is thin**
 
If an email contains only teaser text (1--2 sentences per story) and links to full articles,
use `web_fetch` on 1--2 of the most significant article URLs to get enough detail for a
meaningful summary point. Prioritise fetching when:
- The email body gives only a headline with no detail
- The story appears significant but context is missing
- The linked page is a first-party source (the publication's own site), not a third-party
 
Do not fetch every link -- fetch selectively for the stories that matter most. Skip fetching
for sponsor links, social media, or paywalled publisher sites where the fetch will fail.
 
**3c -- Write the points**
 
For each included email, extract up to **5 key informational points**. Each point should:
 
- Capture one distinct fact, development, insight, or piece of news
- Be a single sentence, written for fast skimming -- short, active, no filler
- Omit greetings, sign-offs, sponsor messages, and calls to action
- Represent something genuinely informative (not "the author says they had a good week")
- For paywalled content where only a teaser is available: summarise the teaser as written,
  no special marker needed
 
Tag each point internally with its source email ID (for deduplication and linking).
 
### Step 4 -- Deduplicate across emails
 
Before rendering, scan all extracted points for overlaps. Two points overlap if they
describe the **same real-world topic, event, or entity**.
